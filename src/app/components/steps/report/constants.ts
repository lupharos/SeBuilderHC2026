...
export const COMPLETION_STEPS = [
  ...
  { step: 11, label: 'Next Steps & Roadmap',       check: (d: CheckData) => d.actionsCount > 0 },
  { step: 12, label: 'Customer Feature Requests',  check: (d: CheckData) => d.frsCount > 0 },
  { step: 13, label: 'Recommended Enhancements',   check: (d: CheckData) => d.enhancementsCount > 0 },
  { step: 14, label: 'License Gap',                check: (d: CheckData) => d.licenseGapCount > 0 },
  { step: 15, label: 'Summary & Review',           check: (d: CheckData) => d.productsSelected },
];
